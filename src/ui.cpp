...
lv_obj_t* rd_clock  = nullptr;                 // in-box clock (idle state)
...
void format_idle_clock(char* out, size_t cap) {
    // Prefer wall-clock once SNTP is synced; fall back to uptime HH:MM.
...
void update_tap_info_panel() {
    ...
    if (rd_selected_ix < 0 || rd_selected_ix >= (int)rd_ac_n ||
        ...
        rd_selected_ix = -1;
        lv_obj_set_style_text_color(rd_info_3, COL_TEXT, 0);
        lv_obj_set_style_text_color(rd_info_4, COL_TEXT, 0);
        lv_obj_align(rd_info_1, LV_ALIGN_TOP_MID, 0, 94);
        lv_obj_align(rd_info_2, LV_ALIGN_TOP_MID, 0, 114);
        lv_obj_align(rd_info_3, LV_ALIGN_TOP_MID, 0, 0);
        lv_obj_align(rd_info_4, LV_ALIGN_TOP_MID, 0, 0);
        lv_obj_align(rd_info_5, LV_ALIGN_TOP_MID, 0, 0);
    ...
    lv_obj_set_style_text_color(rd_info_3, COL_GREEN, 0);
    lv_obj_set_style_text_color(rd_info_4, COL_GREEN, 0);
    lv_obj_align(rd_info_1, LV_ALIGN_TOP_MID, 0, 54);
    lv_obj_align(rd_info_2, LV_ALIGN_TOP_MID, 0, 74);
    lv_obj_align(rd_info_3, LV_ALIGN_TOP_MID, 0, 96);
    lv_obj_align(rd_info_4, LV_ALIGN_TOP_MID, 0, 132);
...
void build_radar_screen() {
    ...
    rd_clock = lv_label_create(rd_info_box);
    // Use the largest built-in font that reliably fits this narrow panel.
    lv_obj_set_style_text_font(rd_clock, &lv_font_montserrat_24, 0);
    lv_obj_set_style_text_color(rd_clock, COL_GREEN, 0);
    lv_obj_set_width(rd_clock, INFO_PANEL_W - 8);
    lv_obj_set_style_text_align(rd_clock, LV_TEXT_ALIGN_CENTER, 0);
    lv_label_set_text(rd_clock, "");
    lv_obj_align(rd_clock, LV_ALIGN_TOP_MID, 0, 46);
...
void radar_place_blips() {
    ...
    char clk[12];
    format_idle_clock(clk, sizeof(clk));
    set_text_if_changed(rd_clock, clk);
    ...
    if (details_active) {
        ...
        lv_obj_add_flag(rd_live, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(rd_clock, LV_OBJ_FLAG_HIDDEN);
    } else {
        lv_obj_remove_flag(rd_count, LV_OBJ_FLAG_HIDDEN);
        lv_obj_remove_flag(rd_clock, LV_OBJ_FLAG_HIDDEN);
