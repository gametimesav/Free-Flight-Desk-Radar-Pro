...
(() => {
  ...
  async function sendCommandHttp(cmd) {
    const path = cmd === 'factory_reset' ? '/api/factory_reset' : '/api/reboot';
    try {
  ...
  function wireInputs() {
    ...
    el.radarSaveBtn.addEventListener('click', async () => {
      ...
      const ok = await sendConfig({ radar: {
        lat, lon,
      ...
      el.radarStatus.textContent = ok ? 'Saved.' : 'Save failed.';
    ...
    el.timezone.addEventListener('change', async () => {
      ...
      const ok = await sendConfig(patch);
    ...
    el.wifiSaveBtn.addEventListener('click', async () => {
      ...
      const ok = await sendConfig(patch);
      el.wifiStatus.textContent = ok
        ? `Saved. Rebooting to join "${ssid}"…`
        : `Save failed. Reboot skipped for "${ssid}".`;
      el.wifiSaveBtn.disabled = !ok;
      if (!ok) return;
    ...
    el.rebootBtn.addEventListener('click', async () => {
      ...
      const ok = await sendCommand('reboot');
      if (!ok) el.wifiStatus.textContent = 'Reboot request failed.';
    });
    el.resetBtn.addEventListener('click', async () => {
      if (!confirm('Erase all settings and reboot?')) return;
      const ok = await sendCommand('factory_reset');
      if (!ok) el.wifiStatus.textContent = 'Factory reset request failed.';
    });
  }
